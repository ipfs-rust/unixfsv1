...
fn walk(blocks: ShardedBlockStore, start: &Cid) -> Result<(), Error> {
    use unixfs_v1::walk::{ContinuedWalk, Walker};
...
impl From<unixfs_v1::walk::Error> for Error {
    fn from(e: unixfs_v1::walk::Error) -> Error {
        Error::Walk(e)
    }
}
...
impl ShardedBlockStore {
    fn as_path(&self, key: &[u8]) -> PathBuf {
        ...
        let encoded = libipld::multibase::Base::Base32Upper.encode(key);
        let len = encoded.len();

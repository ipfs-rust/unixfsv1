...
fn ingest_tar(bytes: &[u8], buffer: &mut Vec<u8>, path: &mut String) {
    ...
    use unixfs_v1::dir::builder::{BufferingTreeBuilder, TreeOptions};
    use unixfs_v1::file::adder::FileAdder;
    use std::io::Read;
    ...
    for entry in entries {
        ...
        if let Some(link_name) = entry.link_name_bytes() {
            ...
            buffer.clear();
            unixfs_v1::symlink::serialize_symlink_block(link_name, buffer);
        ...
        if !path.ends_with('/') {
            ...
            tree.set_metadata(&path[..path.len() - 1], unixfs_v1::Metadata::default())
                .unwrap();
        }
    }
